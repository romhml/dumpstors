...
use dumpstors_lib::models::Keyspace as KeyspaceModel;
use dumpstors_lib::store::{Error, Result};
impl From<Keyspace> for KeyspaceModel {
    fn from(ks: Keyspace) -> KeyspaceModel {
        KeyspaceModel { name: ks.name }
    }
}
...
impl Store {
    ...
    pub fn create_keyspace(&mut self, ks: KeyspaceModel) -> Result<()> {
    ...
    pub fn list_keyspaces(&mut self) -> Result<Vec<KeyspaceModel>> {
        Ok(self
            .keyspaces
            .values()
            .cloned()
...
mod tests {
    ...
    fn load_keyspaces_works() {
        ...
        let ks1 = KeyspaceModel {
            name: String::from("ks1"),
        };
        let ks2 = KeyspaceModel {
    ...
    fn get_keyspace() {
        ...
        assert_eq!(ks1, KeyspaceModel::from(res.clone()));
    }
